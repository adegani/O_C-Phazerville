...
int HemisphereApplet::In(const int ch) {
  return cvmap[ch + io_offset].In();
}

#ifdef __IMXRT1062__
float HemisphereApplet::InF(int ch) {
    return static_cast<float>(In(ch)) / HEMISPHERE_MAX_INPUT_CV;
}
#endif

// Apply small center detent to input, so it reads zero before a threshold
int HemisphereApplet::DetentedIn(int ch) {
...
int HemisphereApplet::SemitoneIn(int ch) {
...
bool HemisphereApplet::Gate(int ch) {
    return trigmap[ch + io_offset].Gate();
}

// --- CV Output methods
void HemisphereApplet::Out(int ch, int value, int octave) {
    frame.Out( (DAC_CHANNEL)(ch + io_offset), value + (octave * (12 << 7)));
}

void HemisphereApplet::SmoothedOut(int ch, int value, int kSmoothing) {
...
void HemisphereApplet::ClockOut(const int ch, const int ticks) {
    frame.ClockOut( (DAC_CHANNEL)(io_offset + ch), ticks);
}
void HemisphereApplet::GateOut(int ch, bool high) {
...
void HemisphereApplet::gfxCursor(int x, int y, int w, int h) { // assumes standard text height for highlighting
  if (EditMode()) {
    gfxInvert(x, y - h, w, h);
  } else if (CursorBlink()) {
    gfxLine(x, y, x + w - 1, y);
...
void HemisphereApplet::gfxSpicyCursor(int x, int y, int w, int h) {
  if (EditMode()) {
    if (CursorBlink())
...
void HemisphereApplet::gfxPos(int x, int y) {
    graphics.setPrintPos(x + gfx_offset, y);
}

void HemisphereApplet::gfxPrint(int x, int y, const char *str) {
    graphics.setPrintPos(x + gfx_offset, y);
    graphics.print(str);
}
void HemisphereApplet::gfxPrint(int x, int y, int num) {
    graphics.setPrintPos(x + gfx_offset, y);
    graphics.print(num);
}
void HemisphereApplet::gfxPrint(const char *str) {
    graphics.print(str);
}
void HemisphereApplet::gfxPrint(int num) {
    graphics.print(num);
}

void HemisphereApplet::gfxPrint(int x, int y, float num, int digits) {
    graphics.setPrintPos(x + gfx_offset, y);
    gfxPrint(num, digits);
}

void HemisphereApplet::gfxPrint(float num, int digits) {
...
void HemisphereApplet::gfxPrint(DigitalInputMap &map) {
  gfxPrintIcon(map.Icon());
  if (map.Gate()) gfxInvert(gfxGetPrintPosX()-8, gfxGetPrintPosY(), 8, 8);
}
void HemisphereApplet::gfxPrint(CVInputMap &map) {
  gfxPrintIcon(map.Icon());
...
void HemisphereApplet::gfxPrint(int x, int y, HS::QuantEngine &q_eng, bool overlay) {
  if (overlay) {
...
void HemisphereApplet::gfxPrint(int x_adv, int num) { // Print number with character padding
...
void HemisphereApplet::gfxEndCursor(bool selected, bool spicy, const char *str) {
    if (selected) {
      if (str) {
...
void HemisphereApplet::gfxPixel(int x, int y) {
    graphics.setPixel(x + gfx_offset, y);
}
void HemisphereApplet::gfxFrame(int x, int y, int w, int h, bool dotted) {
  if (dotted) {
...
void HemisphereApplet::gfxRect(int x, int y, int w, int h) {
    graphics.drawRect(x + gfx_offset, y, w, h);
}
void HemisphereApplet::gfxInvert(int x, int y, int w, int h) {
    graphics.invertRect(x + gfx_offset, y, w, h);
}
void HemisphereApplet::gfxClear(int x, int y, int w, int h) {
    graphics.clearRect(x + gfx_offset, y, w, h);
}
void HemisphereApplet::gfxLine(int x, int y, int x2, int y2) {
    graphics.drawLine(x + gfx_offset, y, x2 + gfx_offset, y2);
}
void HemisphereApplet::gfxLine(int x, int y, int x2, int y2, bool dotted) {
    graphics.drawLine(x + gfx_offset, y, x2 + gfx_offset, y2, dotted ? 2 : 1);
}
void HemisphereApplet::gfxDottedLine(int x, int y, int x2, int y2, uint8_t p) {
    graphics.drawLine(x + gfx_offset, y, x2 + gfx_offset, y2, p);
}
void HemisphereApplet::gfxCircle(int x, int y, int r) {
    graphics.drawCircle(x + gfx_offset, y, r);
}
void HemisphereApplet::gfxBitmap(int x, int y, int w, const uint8_t *data) {
    graphics.drawBitmap8(x + gfx_offset, y, w, data);
}
void HemisphereApplet::gfxBitmapBlink(int x, int y, int w, const uint8_t *data) {
    if (CursorBlink()) gfxBitmap(x, y, w, data);
}
void HemisphereApplet::gfxIcon(int x, int y, const uint8_t *data, bool clearfirst) {
    if (clearfirst) gfxClear(x, y, 8, 8);
    gfxBitmap(x, y, 8, data);
}
void HemisphereApplet::gfxPrintIcon(const uint8_t *data, int16_t w) {
...
void HemisphereApplet::gfxSkyline() {
    ForEachChannel(ch)
    {
...
void HemisphereApplet::gfxHeader(int y) {
  gfxHeader(applet_name(), applet_icon(), y, false);
}

void HemisphereApplet::gfxHeader(const char *str, const uint8_t *icon, int y, bool underline) {
  int x = 1;
  if (icon) {
    gfxIcon(x, y, icon);
    x += 9;
...
void HemisphereApplet::DrawSlider(uint8_t x, uint8_t y, uint8_t len, uint8_t value, uint8_t max_val, bool is_cursor) {
